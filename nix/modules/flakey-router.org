...
* Usage
Import the flakey-router.nix file as a module in your current flake, then add the following to any host you wish to be a router, and customize to taste:
#+begin_example nix
flakey-router = {
  enable       = true;
  lanInterface = "eth1";
...
* Flake Contents
#+begin_src nix :tangle flakey-router.nix
{ config, lib, pkgs, ... }:
