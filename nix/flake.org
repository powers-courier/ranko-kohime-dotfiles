...
:end:
#+begin_src nix :noweb-ref nix-flake-host-auto-discovery
# Get all .nix files from machines directory
hostsFiles = builtins.readDir ./hosts;

# Filter for .nix files and create configurations
hostAutoDiscovery = builtins.listToAttrs (
  builtins.map (name:
    let
      # Remove .nix extension for the attribute name
      hostAutoName = builtins.replaceStrings [".nix"] [""] name;
    in {
      name = hostAutoName;
    ...
    hostAutoDiscovery.${name} == "regular" &&
    nixpkgs.lib.hasSuffix ".nix" name
  ) (builtins.attrNames hostAutoDiscovery))
);
#+end_src
*** Desktops [0/0]
*** DONE Servers [1/1]
