...
** TODO Host Configurations [19/22]
*** DONE mkHost [8/8]
**** DONE Preamble
...
mkHost = {
  ...
  let
    <<mkhost-generator-user-selector>>
...
***** DONE User Selector
#+begin_src nix :noweb-ref mkhost-generator-user-selector
finalHomeUsers =
...
modules = lib.flatten [
  ...
  <<mkhost-generator-module-homemanager>>
  {
...
****** DONE Home Manager
Only activate home-manager if any users are requested

#+begin_src nix :noweb-ref mkhost-generator-module-homemanager
(lib.mkIf (homeUsers != [ ]) [
...
**** TODO Servers [7/9]
***** DONE jelly-proxy Hosts Generator
****** Explanation
******* proxyCount:
...
modules = [
  ...
  ({ pkgs, vars, ... }: {
    networking.hostName = name;
    system.stateVersion = "25.05";
    <<module-jellyfin-proxy>>
  })
];
#+end_src
******* TODO Jellyfin Remote access proxy nodes [5/6]
:properties:
:header-args+: :noweb-ref module-jellyfin-proxy
:end:
******** Host list
