...
#+end_src
** TODO Modules [10/16]
*** DONE All teh Modules, Imported!
#+begin_src nix :tangle modules/modules.nix
{ lib, ... }:
let
...
#+end_src
*** TODO autoUpgrade
#+begin_src nix :tangle modules/autoUpgrade.nix
{ config, lib, ... }: {
...
#+begin_src nix :tangle modules/fwupd.nix
{ config, lib, ... }: {
...
#+end_src
*** Gaming
#+begin_src nix :tangle modules/gaming.nix
{ config, lib, pkgs, ... }:
with lib;
let
  cfg = config.gaming;
in {
  options.gaming = {
    ...
    <<nix-flake-modules-gaming-graphics-nvidia-optimus-prime>>
    <<nix-flake-modules-gaming-steam-conf>>
    <<nix-flake-modules-gaming-lutris-options>>
  ...
  config = mkIf cfg.enable {
    <<nix-flake-modules-gaming-boot>>
    <<nix-flake-modules-gaming-hardware>>
    <<nix-flake-modules-gaming-packages>>
    <<nix-flake-modules-gaming-steam-conf>>
    <<nix-flake-modules-gaming-sound>>
    powerManagement.cpuFreqGovernor = "performance";
    <<nix-flake-modules-gaming-gamemode>>
    <<nix-flake-modules-gaming-networking>>
  };
}
#+end_src
**** Boot options
#+begin_src nix :noweb-ref nix-flake-modules-gaming-boot
boot = {
  kernelParams = [
    "quiet"
    "udev.log_priority=3"
  ...
  <<nix-flake-modules-gaming-graphics-kernel-mod>>
};
#+end_src
**** Game Mode
#+begin_src nix :noweb-ref nix-flake-modules-gaming-gamemode
programs.gamemode = {
  enable = true;
  settings = {
    general = {
...
***** Nvidia
****** Settings
#+begin_src nix :noweb-ref nix-flake-modules-gaming-graphics-nvidia-settings
...
#+end_src
****** Prime
#+begin_src nix :noweb-ref nix-flake-modules-gaming-graphics-nvidia-optimus-prime
graphics = {
  nvidia = {
...
#+end_src
**** Hardware
#+begin_src nix :noweb-ref nix-flake-modules-gaming-hardware
hardware = {
  opengl = {
    enable = true;
    driSupport = true;
  ...
  <<nix-flake-modules-gaming-graphics-nvidia-settings>>
};
#+end_src
**** Load appropriate kernel modules
#+begin_src nix :noweb-ref nix-flake-modules-gaming-graphics-kernel-mod
kernelModules = [
...
#+end_src
**** Networking
#+begin_src nix :noweb-ref nix-flake-modules-gaming-networking
networking = {
  firewall = {
...
#+end_src
**** Packages
#+begin_src nix :noweb-ref nix-flake-modules-gaming-packages
environment.systemPackages = with pkgs; [
  gamemode
  mangohud
  vulkan-tools
...
**** Platforms
***** Lutris
#+begin_src nix :noweb-ref nix-flake-modules-gaming-lutris-options
lutris = {
...
#+end_src
***** Steam
#+begin_src nix :noweb-ref nix-flake-modules-gaming-steam-options
steam = {
  enable = mkEnableOption "Steam and Steam-related tools";
  protonGE = mkEnableOption "ProtonGE custom Proton builds";
};
#+end_src

#+begin_src nix :noweb-ref nix-flake-modules-gaming-steam-conf
...
#+end_src
**** Sound
#+begin_src nix :noweb-ref nix-flake-modules-gaming-sound
sound.enable = true;
...
#+begin_src nix :tangle modules/garbageCollection.nix
{ config, lib, ... }: {
...
#+begin_src nix :tangle modules/nanorc.nix
{ config, lib, ... }: {
...
#+end_src
*** DONE OpenSSH
#+begin_src nix :tangle modules/openssh.nix
{ config, lib, ... }: {
...
#+end_src
*** DONE Tailscale
#+begin_src nix :tangle modules/tailscale.nix
{ config, lib, ... }: {
...
#+end_src
*** DONE zramSwap
#+begin_src nix :tangle modules/zramSwap.nix
{ config, lib, ... }: {
...
#+end_src
* Cruft to incorporate
** TODO System Modules [13/19]
*** TODO Emacs Daemon
...
** TODO Services [9/25]
*** TODO Docker
#+begin_src nix :noweb-ref nix-flake-service-docker
virtualisation.docker.enable = true;
#+end_src
*** TODO Gitlab
#+begin_src nix :noweb-ref nix-flake-service-gitlab
...
#+end_src
* sops-nix mini-flake
#+begin_src nix :tangle sops-nix-dev-shell-flake.nix
{
  inputs = {
    nixpkgs.url = "github:NixOS/nixpkgs/nixos-unstable";
    sops-nix.url = "github:Mic92/sops-nix";
